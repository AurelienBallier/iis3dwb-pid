...
#include "iis3dwb_trigger.h"

LOG_MODULE_REGISTER(iis3dwb, CONFIG_IIS3DWB_LOG_LEVEL);
...
{
	const struct iis3dwb_config *cfg = (const struct iis3dwb_config *)dev->config;
			  ...
			  sensor_trigger_handler_t handler)
{
	const struct iis3dwb_config *cfg = (const struct iis3dwb_config *)dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	struct iis3dwb_data *iis3dwb = (struct iis3dwb_data *)dev->data;
	int16_t raw[3];
...
{
	struct iis3dwb_data *data = (struct iis3dwb_data *)dev->data;
				    ...
				    struct gpio_callback *cb, uint32_t pins)
{
	...
	const struct iis3dwb_config *cfg = (const struct iis3dwb_config *)iis3dwb->dev->config;
...
static int32_t iis3dwb_platform_write(void *handle, uint8_t reg,
                                      const uint8_t *bufp, uint16_t len)
{
    const struct spi_dt_spec *bus = (const struct spi_dt_spec *)handle;
    struct spi_buf tx_buf[2] = {
        { .buf = &reg,   .len = 1 },
        { .buf = (void *)bufp, .len = len },
    };
...
static int32_t iis3dwb_platform_read(void *handle, uint8_t reg,
                                     uint8_t *bufp, uint16_t len)
{
    const struct spi_dt_spec *bus = (const struct spi_dt_spec *)handle;
    uint8_t reg_addr = reg | 0x80; /* add read bit */

    struct spi_buf tx_buf = { .buf = &reg_addr, .len = 1 };
    struct spi_buf_set tx = { .buffers = &tx_buf, .count = 1 };

    struct spi_buf rx_buf = { .buf = bufp, .len = len };
...
/* --- Device instantiation macro for each node --- */
#define IIS3DWB_INIT(inst)                                                      \
	static struct iis3dwb_data iis3dwb_data_##inst;                                \
																				\
	static const struct iis3dwb_config iis3dwb_cfg_##inst = {                      \
		.ctx = {                                                                   \
			.write_reg = iis3dwb_platform_write,                                   \
			.read_reg  = iis3dwb_platform_read,                                    \
		},                                                                         \
		.spi = SPI_DT_SPEC_GET(DT_DRV_INST(inst), SPI_WORD_SET(8) | SPI_TRANSFER_MSB, 0), \
		.range = DT_INST_PROP(inst, range),                                        \
		.odr = DT_INST_PROP(inst, odr),                                            \
																				\
		COND_CODE_1(CONFIG_IIS3DWB_TRIGGER,                                       \
...
/* Instantiate driver for all enabled nodes */
DT_INST_FOREACH_STATUS_OKAY(IIS3DWB_INIT)
